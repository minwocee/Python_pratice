...
def my_average(my_list):   #*여기서 파이썬의 문제 발생 0.5 반올림시 0이 되버림*
    if len(my_list)<=1:    #만약 변량이 1개인 경우 산술평균은 그냥 변량 1개임
        return my_list[0]
    else:
        k=sum(my_list)/len(my_list)   #이때 실수로 결과가 나옴
        if int(10*k)%10<5:
            return int(k)
        else:
            return int(k)+1
    ...
    if len(my_list)<=1:    #만약 변량이 1개인 경우 최빈값은 그냥 변량 1개임
        ...
        for i in range(len(my_list)-1):    #똑같이 검사를 실행
            if my_list[i]==my_list[i+1]:
                ...
                your_counter=1
        if(your_counter==top):
            final_round.append(my_list[-1])
        
    
        if len(final_round)<=1:
            return final_round[0]
        else:
        ...
        #2가지 경우로 분리 모든 요소가 1개씩인경우와 그렇지 않을 경우
